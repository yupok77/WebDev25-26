...
function filterByZip() {
  let zip = document.getElementById("zip").value; // Get the zip code from input
  let output = document.getElementById("output");
  let result = document.getElementById("result");
  ...
  // Loop through the data and filter by zip code
...
function filterByType() {
  let type = document.getElementById("type").value.toLowerCase(); // Get complaint type from input
  let output = document.getElementById("output");
  let result = document.getElementById("result");
  ...
  // Loop through the data and filter by complaint type
  for (let i = 0; i < data.length; i++) {
    let complaint = data[i];
    if (complaint.complaint_type.toLowerCase() === type) {
      build += `<div class="fitted card">
                  <h3>${complaint.complaint_type}</h3>
                  <hr>
                  <p>${complaint.borough}</p>
